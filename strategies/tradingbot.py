...
def recusrion():
    ...
    connection = sqlite3.connect(configure.DB_FILE)
    ...
    def trade():
        if rows:
            ...
            tradeing_client = api.REST(configure.API_Key, configure.API_Secret, configure.URL_ENDPT)
            BASE_URL = configure.URL_ENDPT
            x = 1
            #get active postions 
            x = 1
            tradeing_client = api.REST(configure.API_Key, configure.API_Secret, configure.URL_ENDPT)
